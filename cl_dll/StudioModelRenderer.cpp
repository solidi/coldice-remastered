...
Vector g_vViewModelAimPunch( 0, 0, 0 );
Vector g_vViewModelAimAngles( 0, 0, 0 );
...
void CStudioModelRenderer::StudioCalcAttachments( void )
{
	...
	const bool isViewModel = (m_pCurrentEntity == gEngfuncs.GetViewModel());
	if (isViewModel) {
		g_vViewModelAimPunch = Vector(0, 0, 0);
		g_vViewModelAimAngles = Vector(0, 0, 0);
	}
	...
	for (i = 0; i < m_pStudioHeader->numattachments; i++)
	{
		...
		{
			if (i == 1)
				g_vViewModelAimPunch = pattachment[i].org;
			else if (i == 2)
				g_vViewModelAimAngles = pattachment[i].org;
		}
	}
