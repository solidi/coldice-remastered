	...
	for ( iShot = 1; iShot <= cShots; iShot++ )	
	{
		...
		if ( iBulletType == BULLET_PLAYER_BUCKSHOT || iBulletType == BULLET_PLAYER_BUCKSHOT_SPECIAL )
		{
			...
			if (iBulletType == BULLET_PLAYER_BUCKSHOT_SPECIAL)
			{
				...
				{
					vec3_t vel = vecDir * gEngfuncs.pfnRandomLong(400, 600);
					cl_entity_t *player = gEngfuncs.GetLocalPlayer();
					if (player)
					{
						//gEngfuncs.Con_DPrintf("vel=[%.2f,%.2f,%.2f\n]",player->curstate.velocity[0], player->curstate.velocity[1],player->curstate.velocity[2]);
...
void EV_FireDualSawedOff( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		EV_MuzzleFlash();
		if (clip == 2)
			gEngfuncs.pEventAPI->EV_WeaponAnimation( DUAL_SAWEDOFF_SHOOT_RIGHT, 0 );
		else
			gEngfuncs.pEventAPI->EV_WeaponAnimation( DUAL_SAWEDOFF_SHOOT_LEFT, 0 );
		V_PunchAxis(PITCH, gEngfuncs.pfnRandomFloat(-15.0, -17.0) );
	}
...
void EV_FireDualSawedOffDouble( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( DUAL_SAWEDOFF_SHOOTBOTH, 0 );
		V_PunchAxis(PITCH, gEngfuncs.pfnRandomFloat(-20.0, -22.0) );
	}
...
void EV_FireDualChaingun( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
void EV_FireZapgun( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
void EV_FireDualGlock( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		//if (end)
		//	gEngfuncs.pEventAPI->EV_WeaponAnimation( clip % 2 == 0 ? DUAL_GLOCK_FIRE_LAST_RIGHT : DUAL_GLOCK_FIRE_LAST_LEFT, args->bparam2 );
		//else
			gEngfuncs.pEventAPI->EV_WeaponAnimation( clip % 2 == 0 ? DUAL_GLOCK_FIRE_RIGHT : DUAL_GLOCK_FIRE_LEFT, args->bparam2 );
