...
{
	return g_benchSwitchTimes[ (int)fmin( Bench_GetStage(), LAST_STAGE ) ];
}
	...
	if ( Bench_InStage( THIRD_STAGE ) && strstr( modelname, "ppdemodot" ) )
	{
		...
		if ( Bench_Active() && Bench_InStage( THIRD_STAGE ) )
		{
			...
			if ( dt > 0.0 && dt < 1.0 )
			{
				...
				fZAdjust = fmin( fBounds, fZAdjust );
		...
		*/
		...
		if ( ( NUM_BENCH_OBJ - i - 1 ) < gHUD.m_Benchmark.GetObjects() )
		{
			...
			else
			{
				...
				bench[i].curstate.renderamt = fmin( 255, bench[i].curstate.renderamt );
