...
int CHud :: MsgFunc_Concuss( const char *pszName, int iSize, void *pbuf )
{
	...
	if (m_iConcussionEffect)
		this->m_StatusIcons.EnableIcon("dmg_concuss",0,0);
	else
...
int CHud :: MsgFunc_PlayCSound( const char *pszName, int iSize, void *pbuf )
{
	...
	switch (index)
	{
		...
		case CLIENT_SOUND_NOPE:
			PlaySound("nope.wav", 1);
			break;
		case CLIENT_SOUND_SOCCER:
			PlaySound("soccer.wav", 1);
			break;
		case CLIENT_SOUND_GOL:
			PlaySound("gol.wav", 1);
			break;
		case CLIENT_SOUND_AIRHORN:
			PlaySound("airhorn.wav", 1);
			break;
		case CLIENT_SOUND_CLAPPING:
			PlaySound("goal_1.wav", 1);
			break;
		case CLIENT_SOUND_VOTEGAMEOPTIONS:
			PlaySound("gameoptions.wav", 1);
			break;
	}
	return 1;
}
