...
struct particle
{
	...
	void CalcDistance()
	{
		distance = (v_origin - origin).Length();
	};
};
...
{
public:
	ParticleSystem(int entindex, char *szFilename, float _EmitTime = 0, float _DieTime = 0, Vector _ManualOrigin = Vector(0, 0, 0));
...
//	ParticleType *GetMainType() { return GetType(m_szMainType); }
	...
	void	DrawSystem(std::list <particle *> &List );//vec3_t &right, vec3_t &up);
	...
	bool UpdateParticle( particle *part, float frametime, int messagenum);
	void DrawParticle( particle* part, vec3_t &right, vec3_t &up );
	particle * ManualSpray(int Count, Vector Origin);
	void ManualEmit(float Time, Vector Origin);
...
extern ParticleSystem * MolotovExp;
extern ParticleSystem * MolotovSmoke;
extern ParticleSystem * ExpSmoke;
extern ParticleSystem * ExpSparks;
extern ParticleSystem * BogpSparks;
