...
MutatorInfo sMutators[] = {
	...
	{ "berserker", "fists and chainsaws" },
	{ "bigfoot", "step over anything" },
	{ "bighead", "like Goldeneye" },
	{ "busters", "i ain't afraid of no ghosts" },
	{ "chumxplode", "that's a lot of chum" },
	{ "closeup", "everything is bigger up close" },
	{ "coolflesh", "like popsicles for everyone" },
	{ "cowboy", "yeehaw" },
	{ "crate", "boxed into nostalgia" },
	...
	{ "inverse", "colorblind friendly" },
	{ "invisible", "see nothing, shoot nothing" },
	{ "itemsexplode", "itemsexplode" },
	{ "jack", "in the box" },
	{ "jeepathon", "drive all the jeeps" },
	{ "jope", "it's an inside thing" },
	{ "lightsout", "see nothing, shoot nothing" },
	{ "longjump", "power packed jumps" },
	{ "loopback", "wormhole to your victims" },
	{ "marshmellow", "soft and squishy" },
	{ "maxpack", "more ammo, more fun" },
	...
	{ "plumber", "it's a me, plumber!" },
	{ "portal", "it's a portal gun!" },
	{ "pumpkin", "pumpkinheads!" },
	{ "pushy", "push everyone around" },
...
void TeamFortressViewport::ShowScoreBoard( void )
{
	if (m_pScoreBoard)
	{
		...
		{
			if (g_iUser1)
				m_pScoreBoard->setPos(SBOARD_INDENT_X, SBOARD_INDENT_Y_SHIFT);
			else
				m_pScoreBoard->setPos(SBOARD_INDENT_X, SBOARD_INDENT_Y);
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	if ( iTextToShow == SHOW_MOTD )
	{
		if (!m_szServerName || !m_szServerName[0])
			strcpy( cTitle, "Cold Ice Remastered" );
		else
