...
static const int FLAMETHROWER_NAPALM_POOL_COUNT = 3;
static const float FLAMETHROWER_NAPALM_DAMAGE_PER_TICK = 1.0f;
static const float FLAMETHROWER_NAPALM_RADIUS = 48.0f;
static const float FLAMETHROWER_NAPALM_COOLDOWN = 1.5f;
static const float FLAMETHROWER_NAPALM_STARTUP_TIME = 1.5f;
...
void CFlameThrower::Reload( void )
{
	...
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + FLAMETHROWER_NAPALM_COOLDOWN;
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = m_pPlayer->m_flNextAttack;
	...
	if (iPoolBudget > FLAMETHROWER_NAPALM_POOL_COUNT)
		iPoolBudget = FLAMETHROWER_NAPALM_POOL_COUNT;

	int iPoolsSpawned = CNapalmPool::DeployPools(m_pPlayer, iPoolBudget,
		FLAMETHROWER_NAPALM_DAMAGE_PER_TICK, FLAMETHROWER_NAPALM_RADIUS);
	...
	m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] -= iPoolsSpawned * FLAMETHROWER_NAPALM_POOL_COUNT;
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] < 0)
		m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] = 0;
	...
	PLAYBACK_EVENT_FULL( FEV_GLOBAL | FEV_RELIABLE, m_pPlayer->edict(), m_usFlameThrowerEnd, FLAMETHROWER_NAPALM_STARTUP_TIME,
	...
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + FLAMETHROWER_NAPALM_COOLDOWN;
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = m_pPlayer->m_flNextAttack;
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + FLAMETHROWER_NAPALM_COOLDOWN + 0.5;
#endif
}
