...
void CHAssassin :: Spawn()
{
	...
	pev->effects		= 0;
	pev->health			= (g_pGameRules->IsMultiplayer()) ? gSkillData.hassassinHealth * 2 : gSkillData.hassassinHealth;
...
void CHAssassin :: RunAI( void )
{
	...
	if (pev->renderamt > m_iTargetRanderamt)
	{
		...
		pev->renderamt = fmax( pev->renderamt - 50, m_iTargetRanderamt );
		pev->rendermode = kRenderTransTexture;
	}
	else if (pev->renderamt < m_iTargetRanderamt)
	{
		pev->renderamt = fmin( pev->renderamt + 50, m_iTargetRanderamt );
		if (pev->renderamt == 255)
