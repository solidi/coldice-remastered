...
void CHalfLifeLastManStanding::ClientUserInfoChanged( CBasePlayer *pPlayer, char *infobuffer )
{
	...
	{
		ClientPrint( pPlayer->pev, HUD_PRINTCONSOLE, "[Royale] You're on team '%s' To change, type 'join_blue'\n", pPlayer->m_szTeamName );
		CLIENT_COMMAND(pPlayer->edict(), "model santa\n");
		return;
	}
	if ( !stricmp( "blue", pPlayer->m_szTeamName ) && !stricmp( "iceman", mdls ) )
	{
		ClientPrint( pPlayer->pev, HUD_PRINTCONSOLE, "[Royale] You're on team '%s' To change, type 'join_red'\n", pPlayer->m_szTeamName );
