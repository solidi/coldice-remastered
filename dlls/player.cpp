...
void LinkUserMessages( void )
{
	...
	gmsgSrvOpts = REG_USER_MSG("SrvOpts", -1);     // dynamic-length: chunked server-options manifest
	gmsgVoteSrvOp = REG_USER_MSG("VoteSrvOp", -1); // dynamic-length: open/close + active-item index list
	gmsgSOptFor = REG_USER_MSG("SOptFor", 3);      // BYTE client, BYTE item, BYTE option
...
void CBasePlayer::StartObserver( Vector vecPosition, Vector vecViewAngle )
{
	...
	m_iAssists = 0;
	IsInArena = FALSE;
	// NOTE: m_bWantsToPlay intentionally NOT reset here. StartObserver runs on round transitions
	// and on suck-to-spectator; we must preserve commitment across spawn cycles. Field is
	// initialized in ClientPutInServer (per-connect).
		...
		else
		{
			...
			{	// Start controlling the train!
				...
				if ( pTrain && !(pev->button & IN_JUMP) && FBitSet(pev->flags, FL_ONGROUND) && (pTrain->ObjectCaps() & FCAP_DIRECTIONAL_USE) && pTrain->OnControls(pev) )
				{
					...
					m_iTrain |= TRAIN_NEW;
					pTrain->pev->dmg_inflictor = edict();

					if ( pTrain->Classify() == CLASS_VEHICLE )
					{
						EMIT_SOUND( ENT( pev ), CHAN_ITEM, "plats/vehicle_start1.wav", 0.8, ATTN_NORM );
	...
	if (tr.flFraction != 1.0)
	{
		if ( !FNullEnt( tr.pHit ) )
		{
			...
			if (pEntity && pEntity->Classify() > CLASS_PLAYER)
			{
				...
				m_flStatusBarDisappearDelay = gpGlobals->time + 4.0;
			}
		}
		...
		if ( m_rgpPlayerItems[ i ] && m_rgpPlayerItems[ i ]->m_pPlayer == this )
		{
			...
			while ( pPlayerItem && pPlayerItem->m_pPlayer == this )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					{
						gun->pev->fuser1	= fmax( gun->pev->fuser1 - gpGlobals->frametime, -0.001 );
					}

					// Only decrement if not flagged as NO_DECREMENT
//					if ( gun->m_flPumpTime != 1000 )
				//	{
				//		gun->m_flPumpTime	= fmax( gun->m_flPumpTime - gpGlobals->frametime, -0.001 );
				//	}
...
float IceExplode(CBaseEntity *pAttacker, CBaseEntity *pEntity, int bitsDamageType)
{
	...
	if (!FBitSet(bitsDamageType, DMG_FREEZE))
		return 0;
	...
	UTIL_ScreenFade(pEntity, Vector(0, 113, 230), 1, 2, 128, FFADE_IN);
	...
	if (((CBasePlayer *)pEntity)->m_iFreezeCounter > 45) {
		...
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pEntity->pev->origin );
			...
			// size
			WRITE_COORD( pEntity->pev->size.x );
			WRITE_COORD( pEntity->pev->size.y );
			WRITE_COORD( pEntity->pev->size.z );
			// velocity
			WRITE_COORD( pEntity->pev->velocity.x );
			WRITE_COORD( pEntity->pev->velocity.y );
			WRITE_COORD( pEntity->pev->velocity.z + 64 );
			// randomization
			WRITE_BYTE( 20 );
			// Model
