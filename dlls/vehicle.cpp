...
void CFuncVehicle::Blocked( CBaseEntity *pOther )
{
	...
	float minx = fmin( vBackLeft.x, vBackRight.x );
	float miny = fmin( vBackLeft.y, vBackRight.y );
	float maxx = fmax( vBackLeft.x, vBackRight.x );
	float maxy = fmax( vBackLeft.y, vBackRight.y );
	...
	{
		pOther->TakeDamage( pev, m_pDriver ? m_pDriver->pev : pev, 150, DMG_CRUSH );
	}
}
