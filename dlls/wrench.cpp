...
void CWrench::Smash( int fFatal )
{
	...
	// Skip m_usWrench event playback: its client callback plays a random ATTACK*MISS anim,
	// which would override the WRENCH_THROW2 pose set above. Sounds and decals are handled directly below.
	...
	if ( tr.flFraction < 1.0 )
	{
		...
		if ( pEntity && pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{
				pEntity->pev->velocity = pEntity->pev->velocity + gpGlobals->v_forward * ( fFatal ? 600 : 250 );

				switch ( RANDOM_LONG( 0, 2 ) )
				{
				case 0: EMIT_SOUND( ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hitbod1.wav", 1, ATTN_NORM ); break;
				case 1: EMIT_SOUND( ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hitbod2.wav", 1, ATTN_NORM ); break;
				case 2: EMIT_SOUND( ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hitbod3.wav", 1, ATTN_NORM ); break;
				}
		...
		else
		{
			...
			EMIT_SOUND_DYN( ENT(m_pPlayer->pev), CHAN_BODY,
				fFatal ? "wrench_hit2.wav" : "wrench_hit1.wav",
				fvolbar, ATTN_NORM, 0, 96 + RANDOM_LONG( 0, 3 ) );

			m_pPlayer->m_iWeaponVolume = WRENCH_WALLHIT_VOLUME;
			DecalGunshot( &tr, BULLET_PLAYER_WRENCH );
		}
	}
	else
	{
		EMIT_SOUND_DYN( ENT(m_pPlayer->pev), CHAN_WEAPON,
			"wrench_miss1.wav", 1, ATTN_NORM, 0, 94 + RANDOM_LONG( 0, 0xF ) );
	}
#endif
	...
	else
	{
		...
		if (pEntity)
		{
			...
			if ( pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE )
			{
				...
				{
				case 0:
					EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hitbod1.wav", 1, ATTN_NORM); break;
				case 1:
					EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hitbod2.wav", 1, ATTN_NORM); break;
				case 2:
					EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hitbod3.wav", 1, ATTN_NORM); break;
				}
				m_pPlayer->m_iWeaponVolume = WRENCH_BODYHIT_VOLUME;
				if ( !pEntity->IsAlive() )
				{
		...
		if (fHitWorld)
		{
			...
			{
			case 0:
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hit1.wav", fvolbar, ATTN_NORM, 0, 98 + RANDOM_LONG(0,3));
				break;
			case 1:
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_BODY, "wrench_hit2.wav", fvolbar, ATTN_NORM, 0, 98 + RANDOM_LONG(0,3));
				break;
			}
